...
import type { ScriptNodeData } from "@/components/canvas/lib/canvas-types";
...
export async function POST(
  ...
  try {
    ...
    // Persistir scriptAnalysis en config del nodo (merge con config existente)
    ...
    const newConfig: Record<string, unknown> = {
      ...existingConfig,
      prompt,
      modelId: modelId || existingConfig.modelId,
      modelName: modelDisplayName || existingConfig.modelName,
      temperature: temperature ?? existingConfig.temperature,
      maxOutputTokens: maxOutputTokens ?? existingConfig.maxOutputTokens,
      systemInstruction: systemInstruction ?? existingConfig.systemInstruction,
      scriptAnalysis: result.analysis satisfies ScriptNodeData["scriptAnalysis"],
    };
